...
export async function GET() {
  try {
    ...
    const results = await Promise.all(collections.map(async (collection) => {
      const snap = await db.collection(collection).where("status", "==", "PUBLISHED").limit(100).get();
      return [collection, snap.docs.map((doc) => ({ id: doc.id, ...doc.data() }))] as const;
    }));
    return NextResponse.json({ ok: true, data: Object.fromEntries(results), generatedAt: Date.now() }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {

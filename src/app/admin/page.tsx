...
import { Activity, AlertTriangle, BarChart3, CreditCard, Database, FileAudio, Flag, Gauge, Layers3, ListMusic, LockKeyhole, Shield, ShieldCheck, Tags, Users, WalletCards, Wrench, RefreshCw } from "lucide-react";
...
const formatRevenue = (paise: number) => `₹${(paise / 100).toLocaleString("en-IN", { maximumFractionDigits: 2 })}`;
...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error("Admin dashboard analytics failed", err);
  ...
  const stats = analytics
    ? [
        ["Revenue", formatRevenue(analytics.revenuePaise), CreditCard],
        ["Users", formatNumber(analytics.users), Users],
        ["Tracks", formatNumber(analytics.tracks), Activity],
        ["Subscriptions", formatNumber(analytics.activeSubscriptions), WalletCards],
      ] as const
    : [];
  ...
  return <section className="space-y-7">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {loading && !analytics ? [1,2,3,4].map((key) => <article key={key} className="rounded-2xl border border-white/10 bg-dark-card p-5 shadow-skeuo"><div className="h-4 w-24 animate-pulse rounded bg-white/10"/><div className="mt-5 h-7 w-28 animate-pulse rounded bg-white/10"/><div className="mt-2 h-3 w-40 animate-pulse rounded bg-white/5"/></article>) : stats.map(([label,value,Icon])=><article key={label} className="rounded-2xl border border-white/10 bg-dark-card p-5 shadow-skeuo"><div className="flex items-center justify-between"><span className="text-sm font-bold text-dark-muted">{label}</span><Icon size={19} className="text-accent"/></div><p className="mt-4 text-xl font-black">{value}</p><p className="mt-1 text-xs text-dark-muted">Live server-backed snapshot.</p></article>)}
    </section>

    {error && <section className="flex flex-col gap-3 rounded-2xl border border-red-400/20 bg-red-400/5 p-4 text-sm text-red-200 sm:flex-row sm:items-center sm:justify-between"><span>{error}</span><button onClick={load} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-300/20 px-3 py-2 font-bold"><RefreshCw size={14}/> Retry</button></section>}
